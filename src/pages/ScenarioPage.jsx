    ...
    <div className="space-y-6">
      ...
      {!scenario ? (
        <div className="text-center py-12 bg-white rounded-xl border border-border">
          ...
          <Link to="/activities" className="inline-block mt-4 px-5 py-2 bg-primary text-white rounded-lg text-sm">مكتبة الأنشطة</Link>
        </div>
      ) : (
        <>
          {/* Timeline Bar */}
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className={`bg-white rounded-xl border overflow-hidden ${expandedBlock === i ? ts.border : 'border-border'}`}>
                  <button onClick={() => setExpandedBlock(expandedBlock === i ? null : i)} className="w-full px-5 py-4 flex items-center gap-4 text-right hover:bg-gray-50/50 transition-colors">
                    ...
                    <span className={`px-3 py-1.5 rounded-lg text-xs font-bold ${ts.bg} ${ts.text}`}>{formatDuration(block.duration)}</span>
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        <div className="px-5 pb-5 space-y-4 border-t border-gray-100 pt-4">
                          ...
                          {block.materials?.length > 0 && (
                            <div>
                              <h4 className="text-sm font-bold text-gray-600 mb-2">الأدوات المطلوبة:</h4>
                              <div className="flex flex-wrap gap-2">
                                {block.materials.map((mat, j) => (
                                  <span key={j} className="px-3 py-1.5 rounded-lg text-sm bg-amber-50 text-amber-800 border border-amber-200">{mat}</span>
                                ))}
                              </div>
                            </div>
                          )}
                            ...
                            <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                              ...
                              <ul className="space-y-1.5">
                                {block.tips.map((tip, j) => (
                                  <li key={j} className="text-sm text-blue-700 flex items-start gap-2"><span className="mt-1">•</span><span className="leading-relaxed">{tip}</span></li>
                                ))}
                              </ul>
                            </div>
                          )}
